 ...
 * @description: KUXIONG支付回调
 */
@Service
public class KuxiongPayNotifyService extends BasePayNotify {

    private static final MyLog _log = MyLog.getLog(KuxiongPayNotifyService.class);
    ...
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理KUXIONG支付回调】";
        _log.info("====== 开始处理KUXIONG支付回调通知 ======");
        ...
        try {
            ...
            _log.info("[{}]回调通知参数,data={}", getChannelName(), temp);
                if (StrUtil.equals("0000",(String)temp.get("respCode"),true)) {
                ...
                _log.info("====== 完成处理KUXIONG支付回调通知 ======");
                respString = PayConstant.RETURN_SWIFTPAY_VALUE_SUCCESS;
            }

        }catch (Exception e) {
            _log.error(e, logPrefix + "KUXIONG支付退款处理异常");
        }
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) {
        ...
        // 支付金额
        String total_fee = params.getString("transAmt");
        if (StringUtils.isEmpty(total_fee)) {
            _log.error("KUXIONG Notify parameter total_amount is empty. total_fee={}", total_fee);
        ...
        if (payOrder == null) {
            _log.error("KUXIONG Can't found payOrder form db. payOrderId={}, ", payOrderId);
            payContext.put("retMsg", "Can't found payOrder");
            return false;
        }
        KuxiongConfig payConfig = new KuxiongConfig(getPayParam(payOrder));
        // 验证签名
        if (!checkParam(params, payConfig.getKey())) {
            errorMessage = "check sign failed.";
            _log.error("KUXIONG Notify parameter {}", errorMessage);
        ...
        if (dbPayAmt != outPayAmt) {
            _log.error("db KUXIONG payOrder record payPrice not equals total_amount. total_amount={},payOrderId={}", total_fee, payOrderId);
    ...
    private boolean checkParam (JSONObject params,String key){
        String oldS = params.getString("signature");
