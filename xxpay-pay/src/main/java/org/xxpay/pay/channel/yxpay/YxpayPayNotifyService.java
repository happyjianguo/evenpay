 ...
 * @description: YXPAY支付回调
 */
@Service
public class YxpayPayNotifyService extends BasePayNotify {

    private static final MyLog _log = MyLog.getLog(YxpayPayNotifyService.class);
    ...
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理YXPAY支付回调】";
        _log.info("====== 开始处理YXPAY支付回调通知 ======");
        ...
        try {
            ...
            if (!StringUtils.isEmpty(respString)) {
                ...
                //渠道交易码
                String transaction_id = params.getString("trade_no");
                ...
                _log.info("====== 完成处理YXPAY支付回调通知 ======");
                respString = PayConstant.RETURN_SWIFTPAY_VALUE_SUCCESS;
            }

        }catch (Exception e) {
            _log.error(e, logPrefix + "YXPAY支付退款处理异常");
        }
        retObj.put(PayConstant.RESPONSE_RESULT, respString);
        return retObj;
    }
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) {
        ...
        // 支付金额
        String total_fee = params.getString("total_amount");
        if (StringUtils.isEmpty(total_fee)) {
            _log.error("YXPAY Notify parameter total_amount is empty. total_fee={}", total_fee);
        ...
        if (payOrder == null) {
            _log.error("YXPAY Can't found payOrder form db. payOrderId={}, ", payOrderId);
            payContext.put("retMsg", "Can't found payOrder");
            return false;
        }
        YxpayConfig payConfig = new YxpayConfig(getPayParam(payOrder));
        // 验证签名
        if (!checkParam(params, payConfig.getKey())) {
            errorMessage = "check sign failed.";
            _log.error("YXPAY Notify parameter {}", errorMessage);
            payContext.put("retMsg", errorMessage);
            return false;
        }
        // 核对金额
        long outPayAmt = new BigDecimal(total_fee).longValue()*100;
        long dbPayAmt = payOrder.getAmount().longValue();
        if (dbPayAmt != outPayAmt) {
            _log.error("db YXPAY payOrder record payPrice not equals total_amount. total_amount={},payOrderId={}", total_fee, payOrderId);
    ...
    private boolean checkParam (JSONObject params,String key){
        String oldS = params.getString("sign");
        params.remove("sign");
        params.remove("sign_type");
        Map<String, Object> itemMap = JSONObject.toJavaObject(params, Map.class);
        String signature = SDKUtil.getSign(itemMap,key);
        if (!StrUtil.equals(signature,oldS)){
            return false;
        }
        return true;
    }
